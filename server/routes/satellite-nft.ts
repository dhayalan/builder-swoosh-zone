...
async function uploadToPinata(qrBuffer: Buffer, fileName: string): Promise<string> {
  try {
...
export const generateSatelliteNFT: RequestHandler = async (req, res) => {
  try {
    const { transactionHash, walletAddress, amount }: NFTGenerationRequest = req.body;

    console.log(`🚀 Starting NFT generation for transaction: ${transactionHash}`);
